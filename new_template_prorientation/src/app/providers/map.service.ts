...
export class MapService {
  ...
  public checkDistances = (x,y,array,thresh) => {
    return new Promise(async (resolve,reject) => {
      var boolean = 0;
  ...
  public getArrayCoords = (lat,long,target) => {
    return new Promise(async (resolve,reject) => {
      ...
      reqInterest.addEventListener('readystatechange', () => {
        if(reqInterest.readyState === XMLHttpRequest.DONE) {
          reqInterest.onload = async  () => {
            ...
            var targetNumberNodes = target;
            if(target >= 5){
              targetNumberNodes = 16;
            }
            ...
            for(var i=1;i<elem.length;i++){
              ...
              await this.checkDistances(arrayCoords[i][0], arrayCoords[i][1], arrayCoords1,0.01).then((boolean)=>{
                if(boolean === 0){
            ...
            while(numberNodes > targetNumberNodes + margeNumberNodes || numberNodes < targetNumberNodes - margeNumberNodes){
              ...
              for(var i=1;i<elem.length;i++){
                ...
                await this.checkDistances(arrayCoords[i][0], arrayCoords[i][1], arrayCoords1,thresh).then((boolean)=>{
                  if(boolean === 0){
                    arrayCoords1[ind] = new Array();
                    arrayCoords1[ind][0]= arrayCoords[i][0];
                    arrayCoords1[ind][1]= arrayCoords[i][1];
  ...
  public tsp = (lat,long,target) => {
    ...
    return new Promise(async (resolve,reject) => {
      ...
      var getArray = await this.getArrayCoords(lat, long, target).then(async (resArrayCoords) => {
